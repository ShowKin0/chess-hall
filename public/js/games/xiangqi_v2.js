...
class XiangqiGame extends BaseGame {
  ...
  initialBoard() {
    const red = 'black';   // 先手（下方）
    const blk = 'white';   // 后手（上方）
    const e = null;
    return [
  ...
  allMoves(board, side) {
    ...
    for (let fr = 0; fr < this.rows; fr++) {
      for (let fc = 0; fc < this.cols; fc++) {
        const p = board[fr][fc];
        if (!p || this.sideOf(p) !== side) continue;
        for (let tr = 0; tr < this.rows; tr++) {
          for (let tc = 0; tc < this.cols; tc++) {
            if (this.isValidMoveOnBoard(board, tr, tc, side, { row: fr, col: fc })) {
  ...
  isInCheck(board, side) {
    ...
    const oppMoves = this.allMoves(board, opp);
    return oppMoves.some((m) => m.to[0] === king.row && m.to[1] === king.col);
  }
  ...
  extraInfo(state) {
    ...
    return { lastMove: state.lastMove, checks };
  }
}

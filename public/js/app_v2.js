...
function onWSMessage(msg) {
  switch (msg.type) {
    ...
    case 'error':
      case 'notice':
        toast(msg.type === 'error' ? (msg.message || '出错') : (msg.message || '提示'));
        break;
      // handled above
      break;
    case 'matching':
...
function bindUI() {
  document.querySelectorAll('.menu-btn').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (action === 'single') {
        chooseDifficultyAndStartSingle();
        return;
...
function bindGameListeners() {
  ...
  gameInstance.attachListener((row, col) => {
    ...
    if (room.players.length === 1 && room.turn !== 'black') return; const side = room.players.length === 1 ? 'black' : currentTurnSide();
    if (!side) return;
...
function chooseDifficultyAndStartSingle() {
  ...
  const actions = levels.map((l) => ({
    ...
    onClick: () => {
      ...
      aiThinking = false;
      cancelAITimer();
      sendMsg({ type: 'create_room', mode: 'single' });
    },
  }));
...
function maybeTriggerAI() {
  ...
  aiThinking = true;
    const ti = $('#turn-indicator'); if (ti) ti.textContent = 'AI 思考中…';
  aiTimer = setTimeout(() => {
    aiTimer = null;
    if (!room || room.mode !== 'single') { aiThinking = false; return; }
    if (room.status !== 'playing' || room.turn !== 'white' || gameOver) { aiThinking = false; return; }
    let state = gameInstance.state || gameInstance.buildState(room);
    try {
      const mv = window.InkAI.decide(gameInstance, state, aiLevel, 'white', room.gameType);
      if (!mv) { aiThinking = false; toast('AI 无从落子'); return; }
...
function onRoomUpdate(r) {
  ...
  handlePending(r); maybeTriggerAI();
}
...
function renderRoom() {
  ...
  if (room.mode === 'single' || room.players.length === 2) $('#select-stage').classList.remove('hidden'); else $('#select-stage').classList.add('hidden');
  if (room.players.length === 2) {
    $('#room-tip').textContent = '请选择想要进行的棋局，对方同意后开局';
  } else {
    $('#room-tip').textContent = (room.mode === 'single') ? ('单机模式 · AI 难度「' + aiLevelLabel() + '」：选择棋局开局') : ('等待对手加入…（房 ' + room.id + '）');
  }
}
...
function initResizeObserver() {
  ...
  window.__boardRO = new ResizeObserver(() => {
    if (window.currentView === 'game' && room && gameInstance) {
      resizeCanvas();
      gameInstance.render(room);
...
function updateTurnIndicator(checks) {
  updateStatusBar();
  if (!checks) return;
  const xiangqi = currentGame === 'xiangqi';
  const blackLabel = xiangqi ? '红方' : '黑方';
  ...
  $('#turn-indicator').textContent = cur + '行棋' + extra;
}
...
function updateToolbar() {
  ...
  addTool(bar, '悔棋', () => {
    ...
    if (two) {
      ...
      cancelAITimer(); sendMsg({ type: 'undo' }); sendMsg({ type: 'undo' });
    }
  });
...
function updateResult() {
  ...
  if (room.status === 'finished' || localGameOver()) {
    ...
    else if (room.result === '认输') { title = '你已认输'; desc = room.mode === 'single' ? '此局告负，可再来一局' : '对方认输'; }
    ...
    if (localWinner) {
      title = '胜负已分';
      const label = (localWinner === 'black') ? (currentGame === 'xiangqi' ? '红方' : '黑方') : (currentGame === 'xiangqi' ? '黑方' : '白方');
      desc = label + ' 连五取胜';
    }
    banner.innerHTML = `
